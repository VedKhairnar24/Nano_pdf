...
import { Crop as CropIcon, Download, X, FileText } from "lucide-react";
import { useCropPdf } from "@/hooks/use-crop";
...
export default function Crop() {
  ...
  const cropMutation = useCropPdf();

  // Clean up blob URLs when component unmounts or mutation resets
  useEffect(() => {
    return () => {
      if (cropMutation.data?.downloadUrl) {
        URL.revokeObjectURL(cropMutation.data.downloadUrl);
      }
    };
  }, [cropMutation.data?.downloadUrl]);

  const handleCrop = () => {
    if (!file) {
      toast({
        title: "No file selected",
        description: "Please upload a PDF file to crop.",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-base md:text-lg text-slate-600 max-w-lg mx-auto">
            Trim margins or selected areas of PDF pages.
          </p>
        ...
        <Card className="w-full shadow-xl shadow-slate-200/50 border border-slate-100 p-6 md:p-8">
          <div className="space-y-8">
            ...
            <div className="flex gap-3 pt-4">
              <Button 
                onClick={handleCrop}
                disabled={!file || cropMutation.isPending}
              ...
              >
                {cropMutation.isPending ? (
                  ...
                  <span className="flex items-center gap-2">
                    <CropIcon className="w-5 h-5 mr-2 fill-current" />
                    Crop PDF
                  </span>
                )}
              </Button>
          ...
          <StatusCard 
            status={
              cropMutation.isPending ? "loading" : 
              cropMutation.isSuccess ? "success" : 
              cropMutation.isError ? "error" : "idle"
            }
            message={cropMutation.error?.message}
            downloadUrl={cropMutation.data?.downloadUrl}
            filename={cropMutation.data?.filename}
            onReset={handleReset}
          />
        </Card>
